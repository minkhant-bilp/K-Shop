...
type PurchaseActionSheetProps = {
    visible: boolean;
    onClose: () => void;
    onSubmit: () => void; // ဒါကို Success ဖြစ်မှ ခေါ်တော့မယ်
    item: PackageItem | undefined;
    userId: string;
    zoneId: string;
    gameTitle: string; // 🔥 Added: ဂိမ်းနာမည်ပါမှ Order History မှာပြလို့ရမယ်
};

export default function ActionSheet({ visible, onClose, onSubmit, item, userId, zoneId, gameTitle }: PurchaseActionSheetProps) {

    // 🔥 Store က function တွေ လှမ်းခေါ်မယ်
    const { mmBalance, thBalance, selectedCountry, buyPackage } = useWalletStore();
    ...
    // 🔥 Confirm နှိပ်ရင် အလုပ်လုပ်မယ့် Function
    const handleConfirmPayment = () => {
        if (!isSufficient) return;

        // Store ထဲကို Data တွေ (ပုံအပါအဝင်) ပို့လိုက်ပြီ
        const isSuccess = buyPackage(
            packagePrice,
            selectedCountry,
            gameTitle,   // Game Title (Eg. Mobile Legends)
            item.amount, // Package Name (Eg. 5 Diamonds)
            item.image   // 🔥 Image ကို ဒီနေရာမှာ ထည့်ပေးလိုက်တာပါ
        );

        if (isSuccess) {
            onSubmit(); // အောင်မြင်မှ ပိတ်မယ်/ Navigte လုပ်မယ်
        }
    };
            ...
            >
                ...
                <View style={styles.sheetContent}>
                    ...
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                        onPress={handleConfirmPayment} // 🔥 ပြင်ထားသော Function ကို ချိတ်လိုက်သည်
